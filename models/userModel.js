...
userSchema.methods.sendVerificationEmail = async function () {
  ...
  const mailed = await sendMail(
    user.email,
    `https://best-task-app.onrender.com/verify-email?id=${user._id}&token=${hash}`
  );
  return mailed;
};
...
userSchema.statics.findByCredentials = async function ({
  ...
  if (!user.verified) throw new Error("validation");
  return user;
};
...
userSchema.statics.passwordReset = async function (email) {
  ...
  if (user) {
    ...
    const mailed = await sendMail(
      user.email,
      `https://best-task-app.onrender.com/reset-password?id=${user._id}&token=${hash}`,
      "Reset Your Password",
