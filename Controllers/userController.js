...
module.exports.ensureUniqueUserName = async (req, res) => {
  try {
    ...
    if (user)
      res
        .status(200)
        .json({ err: "username is already taken", success: false });
    else res.status(200).json({ success: true, err: false });
  } catch (e) {
...
module.exports.verifyEmail = async (req, res) => {
  try {
    ...
    if (user.verified) return res.render("ErrorPage");

    if (token !== user.hashed) return res.render("ErrorPage");
...
module.exports.signIn = async (req, res) => {
  try {
    ...
    if (userReq) {
      ...
      res
        .status(201)
        .json({ success: true, error: false, user: userReq, token });
    } else {
...
module.exports.passwordChangeRequest = async (req, res) => {
  try {
    ...
    if (!email) return res.status(400).send({ error: "Invalid Access" });
    console.log("hello");
    const result = await userModel.passwordReset(email);
    console.log(result);
    if (result) return res.status(201).json({ success: true });
    else return res.status(400).json({ error: "not registered" });
  } catch (e) {
...
module.exports.resetPasswordpage = async (req, res) => {
  try {
    ...
    if (!id || !token) return res.render("ErrorPage");
    const user = await userModel.findById(id);
    if (!user.verified || user.hashed !== token) return res.render("ErrorPage");
...
module.exports.resetPassword = async (req, res) => {
  try {
    ...
    if (
      derived.length === allowed.length &&
      derived.every((item) => allowed.includes(item))
    ) {
      const password = await hashPassword(req.body.password);
      const user = await userModel.findOneAndUpdate(
        { _id: req.body.id, hash: req.body.hash },
        { $unset: { hashed: 1 }, $set: { password } }
      );
...
module.exports.findUserById = async (req, res) => {
  try {
    ...
    const result = await userModel.findById(search);
    console.log(result.username);
    res.status(200).json({ result: result.username });
  } catch (e) {
    res.status(500).json({ error: "internal server error" });
  }
};
