...
module.exports.createTask = async (req, res) => {
  ...
  if (
    ...
    req.body.shared ^ req.body.sharewith
  ) {
  ...
  try {
    ...
    if (req.file) task.taskIcon = req.file.buffer;
...
module.exports.editTask = async (req, res) => {
  ...
  try {
    ...
    if (!task) {
      res
        .status(403)
        .json({ error: "You are not allowed to edit others task" });
      return;
    }
...
module.exports.getTaskById = async (req, res) => {
  try {
    ...
    const task = await taskModal.findOne({
      ...
      $or: [
        { owner: req.user._id },
        { "sharewith.username": req.user.username },
      ],
    });
    if (task) res.status(200).json({ task });
    else res.status(404).json({ error: "404! page not found" });
  } catch (e) {
...
module.exports.getAllTasks = async (req, res) => {
  try {
    ...
    if (req.query.pending) match.pending = req.query.pending === "true";
    if (req.query.sortby) {
...
module.exports.getSharedTasks = async (req, res) => {
  try {
    ...
    if (req.query.pending) option.pending = req.query.pending === "true";
    if (req.query.completed) option.completed = req.query.completed === "true";
    const task = await taskModal
      .find({
        $and: [
          {
            $or: [
      ...
      .sort({ updatedAt: req.query.sortBy })
...
module.exports.userTaskcount = async (req, res) => {
  try {
    ...
    res.status(200).json({
      yourTasks: userTasks.length,
      completedTasks: completedTasks.length,
      pendingTasks: pendingTasks.length,
      sharedTasks: sharedTasks.length,
      completedSharedTasks: completedSharedTasks.length,
      pendingSharedTasks: pendingSharedTasks.length,
    });
  } catch (e) {
    res.status(200).json({
      yourTasks: 0,
...
module.exports.markAs = async (req, res) => {
  try {
    ...
    if (req.query.completed === "true") {
      ...
      const task = await taskModal.findOneAndUpdate(
        { _id, owner: req.user._id, completed: false },
        { pending: true }
      );
      res.status(200).json({ task });
    } else if (req.query.incomplete === "true") {
      const task = await taskModal.findOneAndUpdate(
        { _id, owner: req.user._id, completed: true },
        { pending: false, completed: false }
      );
      res.status(200).json({ task });
    } else if (req.query.pending === "false") {
      const task = await taskModal.findOneAndUpdate(
        { _id, owner: req.user._id, pending: true },
        { pending: false, completed: false }
      );
      res.status(200).json({ task });
...
module.exports.searchByHeader = async (req, res) => {
  try {
    ...
    if (req.query.shared) parameter.shared = req.query.shared === "true";
    if (req.query.pending) parameter.pending = req.query.pending === "true";
