...
const CreateTask = () => {
    ...
    const [fdata, setfdata] = useState({ header: "", body: "", durationDate: "", durationTime: "", taskIcon: "", shared: false }); // fdata
    ...
    const submitHandler = async (e) => {
        ...
        if (fdata.header.trim()) {
            ...
            try {
                const res = await Axios.post("http://localhost:5000/create-task", formdata, { withCredentials: true });
            ...
            <div className='row w-100 justify-content-center'>
                <div className='col-7 py-4 shadow'>
                    <div className='container d-flex justify-content-between'>
                        ...
                        <div>
                            <input type="date"
                                form='createTaskform' name="durationDate" title='select end date' onChange={handler} value={fdata.durationDate} /><input type="time" name="durationTime" onChange={handler} value={fdata.durationTime} title='select end time' form='createTaskform' />
                        </div>
                    </div>
                    <hr className='my-1' />
                    <form className='px-3 py-4' id="createTaskform" onSubmit={submitHandler} encType="multipart/form-data" action='' method='POST'>
                        ...
                        <div className='row px-2'>
                            ...
                            <button className='btn btn-success' disabled={(!fdata.header.trim() && (fdata.shared)) ? true : false}>Add Task</button>
                        </div>
                    </form>
                </div>
